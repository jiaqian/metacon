%% load data set
[Head,ContigData]=fastaread('data/species/SpeciesMock_contigs.fasta');
V= tdfread('data/species/SpeciesMock_Contigs_genome_cutup_10K_nodup_filter_1K.csv','|');
V_=struct2cell(V);
contigID=V_{1,1};%type:char,which needs to be converted into string
contigLabel=V_{2,1};%type:double
contigId_ref={};
contigId_head={};
for o=1:size(contigID,1)
    tig=contigID(o,:);
...
contigSampleCovMatURL = 'data/species/covMat_inputtableR.csv';
...
TT=[FF Y];
num_cluster=cal_number_cluster(TT,0.8,1.3);
